...
Matrix4f& Matrix4f::Inverse()
{
	...
	// Check for singular or nearly singular matrix using epsilon (prevents NaN and crashes)
	// Epsilon value (1e-6) handles floating-point precision issues with nearly singular matrices
	const float EPSILON = 1e-6f;
	if (std::abs(det) < EPSILON)
	{
		// Matrix is singular or nearly singular - cannot be inverted
		// Set to identity matrix as safe fallback (prevents NaN propagation)
		InitIdentity();
		return *this;
	}

	// Calculate inverse determinant (safe division after epsilon check)
...
#include "utils.h"  // For fbxToGlmMatrix
#include <vector>
#include "ofbx.h"  // For ofbx::Object
...
{
    std::vector<ofbx::Object*> parentChain;
    
    if (targetNode == nullptr) {
        return parentChain;  // Return empty vector if node is null
    }
    
    // Traverse up the parent chain
    // Note: getParent() returns non-const, but we only read from it
    ofbx::Object* currentParent = const_cast<ofbx::Object*>(targetNode)->getParent();
    
    // First, collect all parents in reverse order (from immediate parent to root)
    std::vector<ofbx::Object*> reverseChain;
    while (currentParent != nullptr) {
        reverseChain.push_back(currentParent);
        currentParent = currentParent->getParent();
    }
    
    // Reverse the chain to get order from root to immediate parent
    for (int i = static_cast<int>(reverseChain.size()) - 1; i >= 0; --i) {
        parentChain.push_back(reverseChain[i]);
    }
...
{
    if (node == nullptr) {
        return localMatrix;  // If no node, local is world
    }
    
    // Get the parent
    // Note: getParent() returns non-const, but we only read from it
    ofbx::Object* parent = const_cast<ofbx::Object*>(node)->getParent();
    
    if (parent == nullptr) {
        // No parent means this is the root - local is world
        return localMatrix;
    }
    
    // Get parent's world matrix (recursive)
    // First get parent's local transform from FBX
    ofbx::Matrix parentLocalFBX = parent->getLocalTransform();
    glm::mat4 parentLocal = fbxToGlmMatrix(parentLocalFBX, false);
    
    // Recursively compute parent's world matrix
    glm::mat4 parentWorld = ComputeWorldMatrix(parent, parentLocal);
...
glm::vec3 WorldToLocal(const glm::vec3& worldPos, const ofbx::Object* parentNode)
{
    if (parentNode == nullptr) {
        // No parent means world space is local space
        return worldPos;
    }
    
    // Get parent's world matrix
    // Note: getLocalTransform() is const, so we can call it on const pointer
    ofbx::Matrix parentLocalFBX = parentNode->getLocalTransform();
    glm::mat4 parentLocal = fbxToGlmMatrix(parentLocalFBX, false);
    glm::mat4 parentWorld = ComputeWorldMatrix(parentNode, parentLocal);
    
    // Invert parent's world matrix to convert from world to local
    glm::mat4 parentWorldInverse = glm::inverse(parentWorld);
    
    // Convert world position to homogeneous coordinates
    glm::vec4 worldPosHomogeneous(worldPos.x, worldPos.y, worldPos.z, 1.0f);
    
    // Apply inverse transformation: Position_local = Inverse(World_parent) * Position_world
    glm::vec4 localPosHomogeneous = parentWorldInverse * worldPosHomogeneous;
    
    // Extract 3D position from homogeneous coordinates
    return glm::vec3(localPosHomogeneous.x, localPosHomogeneous.y, localPosHomogeneous.z);
}
...
glm::mat4 WorldToLocalMatrix(const glm::mat4& worldMatrix, const ofbx::Object* parentNode)
{
    ...
    // Convert: Matrix_local = Inverse(World_parent) * Matrix_world
    return parentWorldInverse * worldMatrix;
}
